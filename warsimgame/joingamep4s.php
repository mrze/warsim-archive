...
if(!$game['option_point_for_ships']){
	redirect('joingame.php?game=' . $_GET['game']);
}

//P4S: Get the amount of credits we have...
$credits = get_credit_amount();
...
if($_GET['mode'] == 'createship'){
	...
	if($_GET['shipid'] && $_GET['shipname'] && is_numeric($_GET['team']) && is_numeric($_GET['shipid'])){
		...
		if(!in_array($row['classid'],$restrict)){
					...
					if($lsp['timestamp'] > (time() - 1200)){
						...
						if(distance($_GET['x'],$_GET['y'],$lsp['x'],$lsp['y']) <= 4){
							...
							if(!$dockedinside){
								...
								redirect('joingamep4s.php?game=' . $_GET['game'] . '&error=' . $error);
							}
						}
					}
				}
			...
			//Check Cost...
			$query = queryme('SELECT `cost`,`classname` FROM `ws_shiprules` WHERE classid = ' . $_GET['shipid']);
			$creditcost = mysql_fetch_assoc($query);
			if($credits <= $creditcost['cost']){
				//Redirect
				$error = 'That Ship costs ' . number_format($creditcost['cost']) . ' Credits, and you only have ' . number_format($credits) . ' Credits. Please Stay within your budget.';
				...
				//See if we already have an account...
				$query = queryme('SELECT `credits` FROM `ws_p4s_accounts` WHERE userid = ' . $_SESSION['userid'] . ' AND gameid = ' . $_GET['game']);
				if(mysql_num_rows($query) == 1){
					//Already have an account, update to remove credits
					$query = queryme('UPDATE `ws_p4s_accounts` SET credits=credits-' . $creditcost['cost'] . ' WHERE userid = ' . $_SESSION['userid'] . ' AND gameid = ' . $_GET['game']);
				} else {
					//Create an account, using default - shipcost for credit amount...
					$query = queryme('INSERT INTO `ws_p4s_accounts` (`credits`,`userid`,`gameid`) VALUES (' . ($credits - $creditcost['cost']) . ',' . $_SESSION['userid'] . ',' . $_GET['game'] . ')');
				}
			}
			...
			//Add Purchase Event...
			$sql = 'INSERT INTO `ws_events` (shipid,time,text) VALUES (' . $new_shipid . ',' . (time() + 2) . ',"You Purchased a ' . $creditcost['classname'] . ' from the market costing ' . number_format($creditcost['cost']) . ' Credits. You have ' . number_format($credits - $creditcost['cost']) . ' credits remaining.");';
			$query = queryme($sql);	
...
$sql = 'SELECT * FROM `ws_shiprules` WHERE display=1 ORDER BY `cost` DESC,`classname` ASC';
$query = queryme($sql);

$lasttype = '';

$page['forminput'] .= '<option onclick="loadshipstats(\'None Selected\',\'Acclamator.gif\',\'None Selected\',\'N/A\',\'N/A\',\'N/A\',\'N/A\',\'N/A\',\'N/A\',\'N/A\',\'N/A\',\'N/A\',\'N/A\')">Type Ships:</option>';
...
while($row = mysql_fetch_assoc($query)){
	if(!in_array($row['classid'],$restrict)){
		...
		//+ Cost...
		if($credits >= ($row['cost'])){
			$page['forminput'] .= '<option value="' . $row['classid'] . '" onclick="loadshipstats(\'' . $row['classname'] . '\',\'' . $row['normalimage'] . '\',\'' . $row['classtype'] . '\',\'' .  $row['maxsquadsize'] . '\',\'' . number_format($row['hullmax']  * $row['maxsquadsize']) . '\',\'' . number_format($row['shieldsmax']  * $row['maxsquadsize']) . '\',\'' . number_format($row['ionicmax']  * $row['maxsquadsize']) . '\',\'' . number_format($row['passengers']  * $row['maxsquadsize']) . '\',\'' . number_format($marines  * $row['maxsquadsize']) . '\',\'' . $row['dedicated_transport']  . '\',\'' . $weapons . '\',\'' . ($row['tractormax']  * $row['maxsquadsize']). '\',\'' . number_format($row['cost']) . '\',\'' . $row['sublight'] . '\')">&raquo; ' .  dot_string($row['classname'],16) . ' - ' . number_format($row['cost']) . '</option>';
		} else {
			$page['forminput'] .= '<option value="' . $row['classid'] . '" onclick="loadshipstats(\'' . $row['classname'] . '\',\'' . $row['normalimage'] . '\',\'' . $row['classtype'] . '\',\'' .  $row['maxsquadsize'] . '\',\'' . number_format($row['hullmax']  * $row['maxsquadsize']) . '\',\'' . number_format($row['shieldsmax']  * $row['maxsquadsize']) . '\',\'' . number_format($row['ionicmax']  * $row['maxsquadsize']) . '\',\'' . number_format($row['passengers']  * $row['maxsquadsize']) . '\',\'' . number_format($marines  * $row['maxsquadsize']) . '\',\'' . $row['dedicated_transport']  . '\',\'' . $weapons . '\',\'' . ($row['tractormax']  * $row['maxsquadsize']). '\',\'' . number_format($row['cost']) . '\',\'' . $row['sublight'] . '\')">&raquo; ' .  dot_string($row['classname'],16) . ' - ' . number_format($row['cost']) . '&curren;</option>';
		}
	}
}
...
while($row =@ mysql_fetch_assoc($query)){
	//Dont display freelance if no freelance option is selected...
...
//P4S stuff:
//amount of credits... fush

$page['p4s'] .= '<br /><span class="green">';
$page['p4s'] .= '<strong>Credits Available To Spend:</strong> <span id="credits">' . number_format($credits);
$page['p4s'] .= '</span></span><br /><br />Ship Cost is shown after the name. If you cannot Afford that type of ship, a &curren; sign will appear next to it.<br /><br />';
